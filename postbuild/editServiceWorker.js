...
const walkSync = (dir, filePaths) => {
  ...
  files.forEach(function(file) {
    ...
    if (fs.statSync(filePath).isDirectory()) {
      ...
      filePaths.push(filePath.replace(/\\/g,"/").replace(mobileRoot,''));
    }
  });
  return filePaths;
};
...
const editProdServiceWorker = () =>{
  const assets = walkSync(mobileRoot,['/']).filter(path=>path!=='service-worker.js');
  const assetsCode = `const assets = ${JSON.stringify(assets)};`;
  const cacheNameCode = `const cacheName = 'ISNCSCI-mobile-${version}';`;
